...
def TransformTensorBilinear(operator, custom_options, tensors, interpreter, features, mat):
    ...
    in_coord_floor = tf.maximum(in_coord_floor, tf.zeros([b, h, w, 2], dtype=tf.int32)) #[b,h,w,XY]
    in_coord_floor = tf.minimum(in_coord_floor, [[[[input_w, input_h]]]]) #[b,h,w,XY]

    in_coord_ceil_ = tf.add(in_coord_floor, tf.ones([b, h, w, 2], dtype=tf.int32)) #[b,h,w,XY]
    in_coord_ceil_ = tf.minimum(in_coord_ceil_, [[[[input_w, input_h]]]]) #[b,h,w,XY]

    in_coord_ceilX = tf.stack([in_coord_floor[:,:,:,1], in_coord_ceil_[:,:,:,0]], axis=3) #[b,h,w,YX] YX for BHWC 
    in_coord_ceilY = tf.stack([in_coord_ceil_[:,:,:,1], in_coord_floor[:,:,:,0]], axis=3) #[b,h,w,YX]

    in_coord_floor = tf.stack([in_coord_floor[:,:,:,1], in_coord_floor[:,:,:,0]], axis=3) #[b,h,w,YX]
    in_coord_ceil_ = tf.stack([in_coord_ceil_[:,:,:,1], in_coord_ceil_[:,:,:,0]], axis=3) #[b,h,w,YX]
