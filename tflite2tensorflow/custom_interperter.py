...
class interpreter():
    def __init__(self, full_json):
        json = full_json

    def get_input_details_from_json():
        indices = self.json['subgraphs'][0]['inputs']
        return_list = []
        for i in indices:
            return_list.append(self.json['subgraphs'][0]['tensors'][i])
        return return_list

    def get_output_details_from_json():
        indices = self.json['subgraphs'][0]['outputs']
        return_list = []
        for i in indices:
            return_list.append(self.json['subgraphs'][0]['tensors'][i])
        return return_list

    def get_operator_details_from_json():
        operators = self.json['subgraphs'][0]['operators']
        i = 0
        for operator in operators:
            opcode_index = operator['opcode_index']
            operator_code = self.json['operator_codes'][opcode_index]
            op_name = op_new_types[operator_code['deprecated_builtin_code']]

            operators[i]['index'] = i
            operators[i]['op_name'] = op_name
            i += 1
